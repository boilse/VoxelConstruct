...
void AVoxelConstructMarchingChunk::Setup()
{
	// Initialize Voxels
	Voxels.SetNum((Size + 1) * (Size + 1) * (Size + 1));
}

void AVoxelConstructMarchingChunk::Generate2DHeightMap(const FVector Position)
{
...
void AVoxelConstructMarchingChunk::Generate3DHeightMap(const FVector Position)
{
	for (int x = 0; x <= Size; ++x)
	{
		for (int y = 0; y <= Size; ++y)
...
void AVoxelConstructMarchingChunk::GenerateMesh()
{
	// Triangulation order
...
void AVoxelConstructMarchingChunk::March(const int X, const int Y, const int Z, const float Cube[8])
{
	int VertexMask = 0;
...
int AVoxelConstructMarchingChunk::GetVoxelIndex(const int X, const int Y, const int Z) const
{
	return Z * (Size + 1) * (Size + 1) + Y * (Size + 1) + X;
}

float AVoxelConstructMarchingChunk::GetInterpolationOffset(const float V1, const float V2) const
{
